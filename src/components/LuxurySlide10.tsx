  ...
  return (
    <div className="w-full h-full bg-[#0B0B0B] relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-16 z-10">
        ...
        {/* Contact info - ultra minimal with glass background */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 2.5, duration: 1.5 }}
          className="space-y-5 backdrop-blur-sm bg-[#0B0B0B]/40 px-10 py-8 rounded-2xl border border-white/10"
        >
          {/* Email */}
          <motion.a
            href="mailto:joaopaulorochareboucas@gmail.com"
            whileHover={{ x: 5 }}
            className="flex items-center gap-4 text-lg text-white/70 hover:text-[#C58B30] transition-colors duration-500"
            style={{ fontFamily: 'Satoshi, sans-serif', fontWeight: 300 }}
          >
            <Mail className="w-5 h-5" strokeWidth={1.5} />
            joaopaulorochareboucas@gmail.com
          </motion.a>
        ...
        >
          <p 
            className="text-xl text-white/60 tracking-[0.3em]"
            style={{ 
      ...
      {/* Expanding circles - reverb visual */}
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
